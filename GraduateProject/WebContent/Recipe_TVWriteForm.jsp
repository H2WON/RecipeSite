...
//유튜브 동영상 보이기================================================================================================================
	...
	function Cooking_AddItem() 
	{
		...
		Recipe_Cooking.innerHTML += "<br>"+"STEP #"
				+ Cooking_cnt
				+ "&nbsp;&nbsp;&nbsp;&nbsp;"
				//+"<input type=text name=CookingDescription placeholder='소고기는 기름기를 떼어내고 적당한 크기로 썰어주세요.'style='height:50px; width:450px;'>"
				...
				+"&nbsp;&nbsp;&nbsp;" 
				+"<img name=Recipe_CookingPhoto id=Recipe_StepPhoto"+Cooking_cnt + " src='' alt=조리사진 style='max-width:20%; height:auto;' />"
				+"&nbsp;&nbsp;"
	...
	FinishedPhoto_cnt = 0;
	 function FinishedPhoto_AddItem()
	 {
		 ...
		 Recipe_FinishedPhoto.innerHTML += "<br>" + "사진 #"
		 	+ FinishedPhoto_cnt
			+ "&nbsp;&nbsp;"
			+ "<img name=Recipe_FinishedPhoto id=Recipe_FinishedPhoto"+ FinishedPhoto_cnt +" src='#' alt='요리완성사진' style='max-width: 50%; height: auto;'/>"
			+ "&nbsp;&nbsp;"
...
<body>
<div align="center"><h1>TV Recipe Writing</h1></div>
<div class="main"> 
    <div class="container tim-container" style="border:1px solid; width:80%; align:center;">
    	<form name="Recipe" action="Recipe_TVWriting_Act.jsp" method="post" enctype="multipart/form-data"  >
	    	<div class="row">
	    	...
	    	<div class="row">
	    		...
	    		<div class="col-md-5 col-sm-10">
	    			<img id="Recipe_ThumbnailName" src='#' alt="Thumbnail" style="max-width:50%; height:auto;" />
	    		</div>
	    	</div>
	    	<div class="row">
	    		<div class="col-md-2 col-sm-4">
	    	...
	    	<div class="row">
	    		...
	    		<div class="col-md-3 col-sm-5">
	    			<select name="Recipe_TypeCategory" style="width: 150px;" >
							<option title="종류별" value="종류별">종류별</option>
							<%
								try {
									Class.forName(driverName); 
									conn = DriverManager.getConnection(dbURL, dbId, dbPass);
									String TypeCategory_sql = "SELECT * FROM TvRecipeTypeCategory GROUP BY TvRecipeTypeCategory_No ORDER BY TvRecipeTypeCategory_No";

									pstmt = conn.prepareStatement(TypeCategory_sql);
									rs = pstmt.executeQuery();
									while (rs.next()) {
										int Sel_RecipeTypeCategory_No = rs.getInt("TvRecipeTypeCategory_No");
										String Sel_RecipeTypeCategory_Name = rs.getString("TvRecipeTypeCategory_Name");
							%>
							<option value=<%=Sel_RecipeTypeCategory_No%>>
								<%=Sel_RecipeTypeCategory_Name%>
							</option>
							<%
								}
	    		...
	    		<div class="col-md-3 col-sm-5">
	    			<select name="Recipe_SituationCategory" style="width: 150px;">
							<option title="상황별" value="상황별">상황별</option>
							<%
								try {
									Class.forName(driverName);
									conn = DriverManager.getConnection(dbURL, dbId, dbPass); 
									String SituationCategory_sql = "SELECT * FROM TvRecipeSituationCategory GROUP BY TvRecipeSituationCategory_No ORDER BY TvRecipeSituationCategory_No";
								
									pstmt = conn.prepareStatement(SituationCategory_sql);
									rs = pstmt.executeQuery();
									while (rs.next()) {
										int Sel_RecipeSituationCategory_No = rs.getInt("TvRecipeSituationCategory_No");
										String Sel_RecipeSituationCategory_Name = rs.getString("TvRecipeSituationCategory_Name");
							%>
							<option value=<%=Sel_RecipeSituationCategory_No%>>
								<%=Sel_RecipeSituationCategory_Name%>
							</option>
							<%
								}
								} catch (SQLException sql) {
	    		...
	    		<div class="col-md-4 col-sm-6">
	    			<select name="Recipe_MaterialCategory" style="width: 150px;">
					<option title="재료별" value="재료별">재료별</option>
							<%
								try {
									Class.forName(driverName);
									conn = DriverManager.getConnection(dbURL, dbId, dbPass); 
									String MaterialCategory_sql = "SELECT * FROM TvRecipeMaterialCategory GROUP BY TvRecipeMaterialCategory_No ORDER BY TvRecipeMaterialCategory_No";

									pstmt = conn.prepareStatement(MaterialCategory_sql);
									rs = pstmt.executeQuery();
									while (rs.next()) {
										int Sel_RecipeMaterialCategory_No = rs.getInt("TvRecipeMaterialCategory_No");
										String Sel_RecipeMaterialCategory_Name = rs.getString("TvRecipeMaterialCategory_Name");
							%>
							<option value=<%=Sel_RecipeMaterialCategory_No%>>
								<%=Sel_RecipeMaterialCategory_Name%>
							</option>
							<%
								}
								} catch (SQLException sql) {
	    	...
	    	<div class="row">
	    		<div class="col-md-12 col-sm-24">
	    			<input type="button" value="재료 추가" onclick="Ingredient_AddItem()" class="btn btn-block btn-lg btn-info btn-round"  style="align:center;">
	    		</div>
	    	</div>
	    	<div class="row">
	    		<div class="col-md-2 col-sm-4">
	    	...
	    	<div class="row">
	    		<div class="col-md-12 col-sm-24">
	    			<input type="button" value="STEP 추가" onclick="Cooking_AddItem()" class="btn btn-block btn-lg btn-info btn-round"  style="align:center;">
	    		</div>
	    	</div>
	    	<div class="row">
	    		<div class="col-md-2 col-sm-4">
	    	...
	    	<div class="row">
	    		<div class="col-md-12 col-sm-24">
	    			<input type="button" value="완성사진 추가" onclick="FinishedPhoto_AddItem()" class="btn btn-block btn-lg btn-info btn-round"  style="align:center;">
	    		</div>
	    	</div>
	    	<div class="row">
	    		<div class="col-md-2 col-sm-4">
	    	...
	    	<div class="row">
	    		...
	    		<div class="col-md-6 col-sm-12" style="text-align:center;">
	    			<input type="reset" width="80" height="40" value="작성취소" class="btn btn-danger"/>
	    		</div>
	    	</div>
    	</form>
	</div>
